...
//get html classes and IDs into js 
...
//?????? how there is no error with country
    country_list.forEach( (country, index) => {
        ...
        //typing onclick event inside html element
        document.getElementById(`${ul_list_id}`).innerHTML += `
            <li onclick="fetchData('${country.name}')"  id="${country.name}" id="${country.name}">
                ${country.name}
            </li>
        `;
    })
...
//num of columns in the countries list
let num_of_ul_lists = 3;

//invoking a function
createCountryList();

// hide the countries list when any country is clicked
chang_country_btn.addEventListener("click", function(){
    //reset the search input each time the countries list in opened
    input.value = "";
    resetCountryList();
    search_country_element.classList.toggle("hide");
});

//show and hide the countries list via the close.svg img
close_list_btn.addEventListener("click", function(){
    search_country_element.classList.toggle("hide");
});

//show and hide the countries list when clicking on the country 
